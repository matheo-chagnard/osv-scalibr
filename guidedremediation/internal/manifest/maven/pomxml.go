...
func (r readWriter) Write(original manifest.Manifest, fsys scalibrfs.FS, patches []result.Patch, outputPath string) error {
	...
	for _, patchPath := range specific.ParentPaths {
		...
		if origDir == "." {
			baseOutputDir = cleanOutDir
		} else if strings.HasSuffix(cleanOutDir, string(filepath.Separator)+origDir) {
			baseOutputDir = cleanOutDir[:len(cleanOutDir)-len(origDir)-1]
		} else if cleanOutDir == origDir {
			baseOutputDir = "."
		} else {
			baseOutputDir = cleanOutDir
		}
